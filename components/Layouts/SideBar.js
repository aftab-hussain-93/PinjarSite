...
const LinkBtn = React.forwardRef(({ onClick, href, title }, ref) => {
    ...
    let styles = "block pt-1 text-left md:py-3 pl-2 align-middle text-gray-50 text-md no-underline sm:border-b-2 sm:border-davysGrey hover:border-primary"
    if (router.pathname === href) {
        styles = "block pt-1 text-left md:py-3 pl-2 align-middle text-gray-50 text-md no-underline sm:border-b-2 border-primary"
    }
    return (
    ...
    return (
        <div className="md:relative mx-auto">
            <ul className="list-reset flex flex-row md:flex-col">
                ...
                <li className="mr-3 flex-1">
                    <Link href='/admin/personalities' passHref><LinkBtn title={`Personality Management`} /></Link>
                </li>
                {/* 
                <li className="mr-3 flex-1">
